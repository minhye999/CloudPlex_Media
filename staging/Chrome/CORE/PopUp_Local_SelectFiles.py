...
class PopUp_Local_SelectFiles(unittest.TestCase):
    ...
    def test_check_tab(self):  # Local / S3 Files 탭 출력 및 동작 확인
        ...
        try:
            ...
            driver.find_element_by_xpath(
                "(.//*[normalize-space(text()) and normalize-space(.)='- or -'])[1]/following::button[1]").click()
            # Local Files 라디오버튼 On 확인
            self.assertEqual("on", driver.find_element_by_name("selectFiles").get_attribute("value"))
            ...
            self.assertEqual("Please select a file to add.", driver.find_element_by_xpath(
                "(.//*[normalize-space(text()) and normalize-space(.)='Add Files'])[1]/preceding::span[1]").text)
            # 파일 부재 시, [Add Files]버튼 출력 확인
            self.assertEqual("Add Files", driver.find_element_by_xpath(
                "(.//*[normalize-space(text()) and normalize-space(.)='Please select a file to add.'])[1]/following::label[1]").text)
        except:
    ...
    def test_check_cancel(self):  # [Cancel]버튼 출력 및 동작 확인
        ...
        try:
            ...
            driver.find_element_by_xpath("(.//*[normalize-space(text()) and normalize-space(.)='Add Files'])[1]/following::button[1]").click()
            # 팝업닫히고, [Local files]버튼 클릭하여 팝업 출력 확인
            driver.find_element_by_xpath(
                "(.//*[normalize-space(text()) and normalize-space(.)='- or -'])[1]/following::button[1]").click()
        except:
    ...
    def test_check_select(self):  # 파일 Select 기능 확인
        ...
        try:
            ...
            #driver.find_element_by_id("upload").send_keys("C:/Users/MZ01 - MINHYE9/Pictures/test_001.mp4")
            # 선택한 파일 확인
            self.assertEqual("test_001.mp4", driver.find_element_by_xpath(
                "(.//*[normalize-space(text()) and normalize-space(.)='Add Files'])[1]/following::span[2]").text)
            ...
            self.driver.find_element_by_xpath("//div[@class='base-file']")
            #self.driver.find_element_by_xpath("//div/span/i[@class='sprite sprite-s3']")
...
def suite():
    ...
    suite.addTest(PopUp_Local_SelectFiles('test_check_title'))
    suite.addTest(PopUp_Local_SelectFiles('test_check_close'))
    suite.addTest(PopUp_Local_SelectFiles('test_check_tab'))
    suite.addTest(PopUp_Local_SelectFiles('test_check_cancel'))
    suite.addTest(PopUp_Local_SelectFiles('test_check_select'))
    return suite
