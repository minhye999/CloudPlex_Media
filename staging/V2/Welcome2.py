...
class Welcome(unittest.TestCase):
    ...
    def test_check_text(self):
        ...
        try:
            ...
            print('TEST FAIL : test_check_text')
            logging.basicConfig(stream=sys.stderr, level=logging.error)  # 로그 출력
            now = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
            self.driver.save_screenshot('../../../staging/V2/Test_Results/Screenshots/test_Title-%s.png' % now)
        else:
            print('TEST PASS : test_check_text')
...
def suite():
    ...
    suite.addTest(Welcome('test_check_text'))
    suite.addTest(Welcome('test_check_signIn'))
    suite.addTest(Welcome('test_check_createAccount'))
    return suite

if __name__ == "__main__":
    runner = HtmlTestRunner.HTMLTestRunner(output="../../../staging/V2/Test_Results/Reports")
    runner.run(suite())